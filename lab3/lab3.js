...
describe("YandexMarket test", async () => {
    ...
    it(
        ...
        withErrorHandling(
            async () => {
                if (await yandexMarketPage.isElementPresent("//button[@title='Удалить из избранного']")) {
                    await yandexMarketPage.clickElement("//button[@title='Удалить из избранного']");
                } else {
                    console.log("Элемент 'Удалить из избранного' не найден");
                }
            },
    ...
    it(
        ...
        withErrorHandling(
            async () => {
                if (await yandexMarketPage.isElementPresent("//h3[@class='G_TNq _2SUA6 _33utW _13aK2 h29Q2 _1A5yJ']") &&
                    await yandexMarketPage.isElementPresent("//span[@class='_1ArMm']")) {
                    const [title, price] = await yandexMarketPage.getFavorites();
                    if (title !== firstElem[0][0] || price !== firstElem[0][1]) {
                        throw new Error(`Expected title: ${firstElem[0][0]}, price: ${firstElem[0][1]}. Actual title: ${title}, price: ${price}`);
                    }
                } else {
    ...
    it(
        ...
        withErrorHandling(
            async () => {
                ...
                const savedText = await yandexMarketPage.getTextFromElement("//div[@class='_3wd6p _1ehmv']");
